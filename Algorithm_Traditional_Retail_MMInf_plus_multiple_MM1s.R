...
max_throughput <- Optimum_Capacity
...
for (t in 1:dim(N1)[1]) {
  ...
  max_throughput[t,1] <- max(temp$Throughput)
...
write.csv(Outcomes, file = "Outcomes_LD_SMM1.csv")


ggplot(max_throughput, aes(time, Max_Throughput)) +
  xlab("time (h)")+
  ylab("Max. Throughput (Customers/h)")+
  geom_point()+
  geom_line()
